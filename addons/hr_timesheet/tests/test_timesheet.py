...
class TestTimesheet(TestCommonTimesheet):
    ...
    def test_minutes_computing_after_timer_stop(self):
        ...
        timesheet_1.with_user(self.user_employee)._get_record_timer().timer_start = now - timedelta(seconds=28)
        ...
        now = datetime.now()
        timesheet_1.with_user(self.user_employee).action_timer_start()
        timesheet_1.with_user(self.user_employee)._get_record_timer().timer_start = now - timedelta(minutes=1, seconds=28)
        timesheet_1.with_user(self.user_employee).action_timer_stop()
