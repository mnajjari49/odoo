odoo.define('website_form.s_website_form_001', function (require) {
'use strict';
...
publicWidget.registry.s_website_form_001 = publicWidget.Widget.extend({
    selector: '.s_website_form[data-vjs="001"] form',
    xmlDependencies: ['/website_form/static/src/xml/website_form.xml'],

    events: {
        'click .s_website_form_send': 'send',
    },
    ...
    start: function () {
        ...
        this.$target.find('.s_website_form_datetime').datetimepicker(datepickers_options);

        // Adapt options to date-only pickers
        datepickers_options.format = time.getLangDateFormat();
        this.$target.find('.s_website_form_date').datetimepicker(datepickers_options);
    ...
    destroy: function () {
        ...
        this.$target.find('#s_website_form_result').empty();
    ...
    send: function (e) {
        ...
        this.$target.find('.s_website_form_send').addClass('disabled');  // Prevent users from crazy clicking

        var self = this;

        self.$target.find('#s_website_form_result').empty();
        ...
        .then(function (result_data) {
            ...
            if (!result_data.id) {
                ...
                switch (successMode) {
                    case 'redirect':
                        $(window.location).attr('href', self.$target[0].dataset.successPage);
                        break;
                    case 'message':
    ...
    check_error_fields: function (error_fields) {
        ...
        var form_valid = true;
        // Loop on all fields
        this.$target.find('.s_website_form_field').each(function (k, field){
            var $field = $(field);
            var field_name = $field.find('.col-form-label').attr('for');

            // Validate inputs for this field
            var inputs = $field.find('.s_website_form_input:not(#editable_select)');
            var invalid_inputs = inputs.toArray().filter(function (input, k, inputs) {
                ...
                } else if ($(input).hasClass('s_website_form_date')) {
                    if (!self.is_datetime_valid(input.value, 'date')) {
                        return true;
                    }
                } else if ($(input).hasClass('s_website_form_datetime')) {
    ...
    update_status: function (status) {
        ...
        var $result = this.$('#s_website_form_result');
