...
class TestSyncGoogle2Odoo(SavepointCase):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence.rule']._sync_google2odoo(google_recurrence)
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence.rule']._sync_google2odoo(GoogleEvent([values]))
        recurrence = self.env['calendar.recurrence.rule'].search([('google_id', '=', values.get('id'))])
    ...
    def test_recurrence_exdate(self):
        ...
        self.sync(events)
        recurrence = self.env['calendar.recurrence.rule'].search([('google_id', '=', recurrence_id)])
        self.assertTrue(recurrence, "it should have created a recurrence")
        events = recurrence.calendar_event_ids.sorted('start')
    ...
    def test_existing_recurrence_first_updated(self):
        recurrence_id = "4c0de517evkk3ra294lmut57vm"
        recurrence = self.env['calendar.recurrence.rule'].create({
            'google_id': recurrence_id,
            'rrule': 'FREQ=WEEKLY;WKST=SU;COUNT=3;BYDAY=MO',
            'base_calendar_event_id': self.env['calendar.event'].create({'name': 'coucou', 'allday': True, 'start': datetime(2020, 1, 6), 'stop': datetime(2020, 1, 6)}).id,
        })
    ...
    def test_recurrence_moved(self):
        ...
        self.env['calendar.recurrence.rule'].with_context(caca=True)._sync_google2odoo(GoogleEvent([values]))
        recurrence = self.env['calendar.recurrence.rule'].search([('google_id', '=', recurrence_id)])
        events = recurrence.calendar_event_ids.sorted('start')
