...
class Event(models.Model):
    ...
    sale_total_price = fields.Monetary('Total Sales', compute='_compute_sale_total_price')
    ...
    def _compute_from_event_type(self):
        ...
        for event in self:
            if event.event_type_id.use_ticketing:
                event.event_ticket_ids = [(5, 0, 0)] + [
                    (0, 0, {
                        'name': event.name and _('Registration for %s') % event.name or ticket.name,
                        'product_id': ticket.product_id.id,
                        'price': ticket.price,
                    })
                    for ticket in event.event_type_id.event_ticket_ids]
