...
const actions = {
    ...
    /**
     * Close the specified chat window.
     *
     * @param {Object} param0
     * @param {function} param0.dispatch
     * @param {Object} param0.state
    ...
    /**
     * Create an attachment from data received form the server (or "temporary"
     * files, which are attachments that are currently being uploaded).
     *
     * @param {Object} param0
     * @param {function} param0.dispatch
     * @param {Object} param0.state
    ...
    createAttachment({ dispatch, state }, data) {
        ...
        if (isTemporary || !temporaryAttachmentLocalId) {
            if (composerLocalId) {
                dispatch('_linkAttachmentToComposer', composerLocalId, attachment.localId);
            }
            if (threadLocalIds) {
    ...
    async joinChannel(
        ...
        const threadLocalId = dispatch('_createThread', Object.assign({}, data));
        if (state.threads[threadLocalId].is_minimized) {
            dispatch('openThread', threadLocalId, { chatWindowMode: 'last' });
        }
        if (autoselect) {
    ...
    /**
     * Load the specific thread cache. In particular, this fetches the most
    ...
    /**
     * Replace a chat window with another one. The former is closed, while the
    ...
    /**
     * Toggle the open state of the messaging menu.
     *
     * @param {Object} param0
     * @param {function} param0.dispatch
     * @param {Object} param0.state
    ...
    /**
     * Update the state of discuss app in the store.
     *
     * @param {Object} param0
     * @param {function} param0.dispatch
     * @param {Object} param0.state
    ...
    /**
     * Update the messaging menu state in the store.
     *
     * @param {Object} param0
     * @param {Object} param0.state
     * @param {Object} changes
     ...
     */
    makeChatWindowVisible({ dispatch, state }, chatWindowLocalId) {
        const cwm = state.chatWindowManager;
        const {
            length: l,
    ...
    _openChatWindow(
        ...
        if (cwm.chatWindowLocalIds.includes(chatWindowLocalId)) {
            ...
            if (
                ...
                dispatch('makeChatWindowVisible', chatWindowLocalId);
            }
        } else {
            // new chat window
