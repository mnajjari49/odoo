...
publicWidget.registry.websiteSlidesCourseContentManager = publicWidget.Widget.extend({
    ...
    _attachWidgetsToAddedContent: function (categoryCreated){
        var proms = [];
        proms.push(new websiteSlidesArchive(this, arguments).attachTo(this.$('.o_wslides_js_slide_archive.created-slide')));
        proms.push(new websiteSlidesTogglePreview(this, arguments).attachTo(this.$('.o_wslides_js_slide_toggle_is_preview.created-slide')));
        if (categoryCreated){
            proms.push(new publicWidget.registry.websiteSlidesUpload(this, arguments).attachTo(this.$('.o_wslides_js_slide_upload.created-category')));
        }
    ...
    _attachWidgetsToInitialContent: function (){
        ...
        this.$('.o_wslides_js_slide_upload').map(function (){
            proms.push(new websiteSlidesUpload(self, arguments).attachTo($(this)));
        });
        this.$('.o_wslides_js_slide_section_add').map(function (){
            proms.push(new websiteSlidesCategoryAdd(self, arguments).attachTo($(this)));
        });
        this.$('.o_wslides_js_slide_archive').map(function (){
            proms.push(new websiteSlidesArchive(self, arguments).attachTo($(this)));
        });
        this.$('.o_wslides_js_slide_toggle_is_preview').map(function (){
            proms.push(new websiteSlidesTogglePreview(self, arguments).attachTo($(this)));
        });
        proms.push(this.dragAndDropWidget.attachTo(self.$('.o_wslides_slides_list')));
        return Promise.all(proms);
    },
    /**
     *
