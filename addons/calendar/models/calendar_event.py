...
from odoo.addons.calendar.models.calendar_recurrence_rule import weekday_to_field, RRULE_TYPE_SELECTION, END_TYPE_SELECTION, MONTH_BY_SELECTION, WEEKDAY_SELECTION, BYDAY_SELECTION
...
class Meeting(models.Model):
    ...
    start = fields.Datetime('Start', required=True, tracking=True, help="Start date of an event, without time for full days events")
    stop = fields.Datetime('Stop', required=True, tracking=True, compute='_compute_stop', inverse='_inverse_stop', store=True,
    ...
    recurrence_id = fields.Many2one('calendar.recurrence.rule', string="Recurrence Rule", index=True)
    ...
    def _compute_recurrence(self):
        ...
        defaults = self.env['calendar.recurrence.rule'].default_get(recurrence_fields)
        for event in self:
            values = {
            ...
            event.update({**false_values, **defaults, **values})
    ...
    def _attendees_values(self, partner_commands):
        ...
        # LUL TODO clean that
        google_internal_event_id = self._context.get('google_internal_event_id')
        values = {'google_internal_event_id': google_internal_event_id} if google_internal_event_id else {}

        attendee_commands += [
            [0, 0, dict(values, partner_id=partner_id)]
    ...
    def _apply_recurrence_values(self, values, future=True):
        ...
        if not values:
            return self.browse()
        recurrence_vals = []
        to_update = self.env['calendar.recurrence.rule']
        for event in self:
            if not event.recurrence_id:
                recurrence_vals += [dict(values, base_calendar_event_id=event.id, calendar_event_ids=[(4, event.id)])]
            elif future:
                to_update |= event.recurrence_id._split_from(event, values)
        self.recurrency = True
        to_update |= self.env['calendar.recurrence.rule'].create(recurrence_vals)
    ...
    def _break_recurrence(self, future=True):
        ...
        """
        recurrences_to_unlink = self.env['calendar.recurrence.rule']
    ...
    def write(self, values):
        ...
        if update_recurrence:
            if values.get('recurrency') is False:
        ...
        if recurrence_update_setting != 'self_only':
