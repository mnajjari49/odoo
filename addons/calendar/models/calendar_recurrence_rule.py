...
class RecurrenceRule(models.Model):
    _name = 'calendar.recurrence.rule'
    ...
    def _apply_recurrence(self):
        ...
        for recurrence in self.filtered('base_calendar_event_id'):
            event = recurrence.base_calendar_event_id or recurrence._get_first_event(include_outliers=False)
        ...
        self.env['calendar.event'].with_context(no_mail_to_attendees=True).create(event_vals)
        return detached_events

    def _split_from(self, event, recurrence_values={}):
        """
        ...
        return self.create({
            **values,
            **recurrence_values,
            'base_calendar_event_id': event.id,
    ...
    def _get_base_event(self):
        ...
        base = self.base_calendar_event_id
        if base.active and base not in outliers:
            return base
        return (self.calendar_event_ids - outliers)[:1] or self.calendar_event_ids[:1]
    ...
    def _get_occurrences(self, dtstart):
        ...
        start_timezoned = pytz.utc.localize(dtstart).astimezone(timezone)
        ...
        occurences = self._get_rrule(dtstart=start_timezoned.replace(tzinfo=None))
    ...
    def _excluded_occurences(self):
        return set(self.env['calendar.event'].search_read([('recurrence_id', 'in', self.ids), ('active', '=', False)], ['start']))
    ...
    def _get_rrule(self, dtstart=None):
        ...
        if freq == 'monthly' and self.month_by == 'date':  # e.g. every 15th of the month
            ...
            rrule_params['byweekday'] = self._get_week_days()
        ...
        if self.end_type == 'count':  # e.g. stop after X occurence
            ...
            until = self.until if self._is_allday() else datetime.combine(self.until, time.max)
