        ...
        if self.cash_rounding_id and self.type in ('out_invoice', 'out_refund'):
            ...
            if not self.currency_id.is_zero(rounding_amount):
                if self.cash_rounding_id.strategy == 'biggest_tax':
                    ...
                    if rounding_amount > 0.0 and self.cash_rounding_id.loss_account_id:
                        account_id = self.cash_rounding_id.get_loss_account_id().id
                    else:
                        account_id = self.cash_rounding_id.get_profit_account_id().id
