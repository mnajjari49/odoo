    ...
    def _sub_select(self):
        sub_select_str = """
            SELECT aml.id AS id,
                ...
                (CASE WHEN am.l10n_in_place_of_supply_id = cp.state_id or p.id IS NULL
                    THEN 'Intra State'
                    WHEN am.l10n_in_place_of_supply_id != cp.state_id and p.id IS NOT NULL
                    THEN 'Inter State'
                    END) AS supply_type,
                ...
                CASE WHEN tag_rep_ln.account_tax_report_line_id IN
                    ...
                    END AS price_total,
                (CASE WHEN (aj.type = 'sale' AND am.type != 'out_refund') or (aj.type = 'purchase' AND at.l10n_in_reverse_charge AND am.type != 'in_refund') THEN -1 ELSE 1 END) AS amount_sign,
    ...
    def _from(self):
        from_str = """
            FROM account_move_line aml
                ...
                LEFT JOIN res_country_state ps ON ps.id = am.l10n_in_place_of_supply_id
    ...
    def _where(self):
        return """
                WHERE am.state = 'posted'
                    AND tag_rep_ln.account_tax_report_line_id in (SELECT res_id FROM ir_model_data WHERE module='l10n_in' AND name in ('tax_report_line_igst','tax_report_line_cgst','tax_report_line_sgst','tax_report_line_igst_rc','tax_report_line_cgst_rc','tax_report_line_sgst_rc'))
        """
