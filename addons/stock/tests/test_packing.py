...
class TestPacking(TransactionCase):
    ...
    def test_pack_in_receipt_two_step(self):
        ...
        self.assertEqual(
            ...
            "The package destination location must be the one from the first putaway.")
        self.assertEqual(
            internal_package.move_line_ids[0].location_dest_id.id,
            putaway.product_location_ids[0].fixed_location_id.id,
            "The move line destination location must be the one from the first putaway.")
        self.assertEqual(
