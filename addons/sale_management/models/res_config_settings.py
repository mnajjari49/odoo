...
class ResConfigSettings(models.TransientModel):
    ...
    def create(self, values):
        if 'group_sale_order_template' in values and not values.get('group_sale_order_template'):
            values['company_so_template_id'] = None
            res = super().create(values)
            # Reset all defaults when the feature is disabled
