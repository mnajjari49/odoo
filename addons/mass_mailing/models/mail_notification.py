...
class MailNotification(models.Model):
    ...
    campaign_id = fields.Many2one('utm.campaign', compute='_compute_campaign_id', string='Campaign', index=True, store=True, default=False)
    # Bounce and tracking
    ...
    @api.depends('mass_mailing_id.campaign_id')
    def _compute_campaign_id(self):
        for notif in self:
            notif.campaign_id = notif.mass_mailing_id and notif.mass_mailing_id.campaign_id or False
    ...
    def create(self, values_list):
        for values in values_list:
            ...
            if values.get('mass_mailing_id'):
                ...
                status_info = self._get_notification_status(values)
                if status_info:
