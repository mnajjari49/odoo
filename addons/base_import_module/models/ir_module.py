...
class IrModule(models.Model):
    ...
    def module_uninstall(self):
        """Delete an ir_module_module record completely if it was an imported one.
        
        The rationale behind this is that an imported module *cannot* be reinstalled
        anyway, as it requires the data files. Any attempt to install it again will
        simply fail without trace."""
        res = super().module_uninstall()
        modules_to_delete = self.filtered('imported')
        _logger.info('deleting imported modules upon uninstallation: %s', ','.join(modules_to_delete.mapped('name')))
