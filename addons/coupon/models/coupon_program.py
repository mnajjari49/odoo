...
class CouponProgram(models.Model):
    ...
    def unlink(self):
        ...
        return super(CouponProgram, self).unlink()

    def toggle_active(self):
        super(CouponProgram, self).toggle_active()

        # Disable the unused coupons linked to the deactivated programs.
        # No need for further filter based on program_type or program_applicability
        # because only those programs-that-can-have-coupons have coupons.
        deactivated_programs = self.filtered(lambda p: not p.active)
        deactivated_programs.coupon_ids.filtered(lambda x: x.state != 'used').write({'state': 'expired'})

        # We also toggle the linked discount line product.
        self.mapped('discount_line_product_id').toggle_active()
