...
class PurchaseOrder(models.Model):
    ...
    mto_mrp_production_count = fields.Integer("Count of MO Source", compute='_compute_mto_mrp_production_count', store=True,
                                              groups='mrp.group_mrp_user')

    @api.depends('order_line.move_dest_ids.group_id.mrp_production_id')
    def _compute_mto_mrp_production_count(self):
        for purchase in self:
            purchase.mto_mrp_production_count = len(purchase.order_line.move_dest_ids.group_id.mrp_production_id)
