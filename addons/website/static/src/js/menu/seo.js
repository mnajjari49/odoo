...
var MetaImageSelector = Widget.extend({
    ...
    init: function (parent, data) {
        ...
        data.pageImages.unshift(data.socialImage);
        this.images = _.uniq(_.compact(data.pageImages));
        this.pageImagesSize = data.pageImagesSize;
     ...
     * @private
    */
   _excludeSmallerImages: function () {
        // smaller then 200x200 size images remove for seo
        var excludeImages = _.pluck(_.filter(this.pageImagesSize, function (image) {
            return image.naturalWidth < 200 || image.naturalHeight < 200;
        }), 'src');
        return _.difference(this.images, excludeImages);
    },
    /**
    ...
    */
    _updateTemplateBody: function () {
        this.$el.empty();
        this.images = _.uniq(this._excludeSmallerImages());
...
var SeoConfigurator = Dialog.extend({
    ...
    start: function () {
        ...
        this.disableUnsavableFields().then(function () {
            ...
            self.metaImageSelector = new MetaImageSelector(self, {
                ...
                pageImagesSize: self.htmlPage.images(),
    ...
    disableUnsavableFields: function () {
        ...
        return this.loadMetaData().then(function (data) {
            ...
            self.socialImage = data.has_default_share_image ? _.str.sprintf('/web/image/website/%s/social_default_image', odoo.session_info.website_id) : '';
        ...
        return new Promise(function (resolve, reject) {
            if (!obj) {
                ...
                var fields = ['website_meta_title', 'website_meta_description', 'website_meta_keywords'
                                ,'website_meta_og_img', 'has_default_share_image'];
