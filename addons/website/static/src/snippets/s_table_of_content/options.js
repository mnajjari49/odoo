...
options.registry.TableOfContent = options.Class.extend({
    ...
    _generateNav: function (ev) {
        ...
        _.each($headings, el => {
            ...
            $('<a>').attr('href', "#" + id)
                    .addClass('list-group-item list-group-item-action py-2 border-0 rounded-0')
                    .text($el.text())
                    .appendTo($nav);
            $el.attr('id', id);
    ...
    /**
     * Generate the navbar if the content changed.
     * 
     * @private
     */
    _observeContentChange: function (ev) {
        const self = this;
        ...
        switch (methodName) {
            ...
            case 'navbarPosition': {
                ...
                if ($navbar.hasClass('s_table_of_content_horizontal_navbar')) {
                    ...
                    let $mainContent = this.$target.find('.s_table_of_content_main');
