...
QUnit.module('fields', {}, function () {
    ...
    QUnit.module('relational_fields', {
        ...
        QUnit.test('one2many list: create/delete action disabled on parent field based condition', async function (assert) {
            assert.expect(6);
            ...
            assert.containsNone(form, 'td.o_list_record_remove button',
                "should not have trash bin icons in readonly");

            await testUtils.form.clickEdit(form);
            assert.containsOnce(form, '.o_field_x2many_list_row_add',
                '"Add an item" link should be available in edit mode');
            assert.hasClass(form.$('td.o_list_record_remove button').first(), 'fa fa-trash-o',
                "should have trash bin icons in edit mode");
            ...
            assert.containsNone(form, 'td.o_list_record_remove button',
                "should have trash bin icons if bar field is False");
        ...
        QUnit.test('one2many kanban: create/delete action disabled on parent field based condition', async function (assert) {
            assert.expect(7);
            ...
            assert.containsNone($(document), $('.modal .modal-footer .o_btn_remove'),
                'There should not be a Remove Button inside modal');
            assert.containsOnce($(document), '.modal .modal-footer .o_form_button_cancel',
                'There should be a modal having Remove Button');

            await testUtils.dom.click($('.modal .modal-footer .o_form_button_cancel'));
            await testUtils.form.clickEdit(form);
            assert.containsOnce(form, '.o-kanban-button-new',
                '"Add" button should be available in edit mode');
            ...
            await testUtils.dom.click(form.$('.o_field_widget[name="bar"] input').first());
            assert.containsNone(form, '.o-kanban-button-new',
                '"Add" button should not be available if bar field is False');

            await testUtils.dom.click(form.$('.oe_kanban_global_click').first());
            assert.containsNone($(document), $('.modal .modal-footer .o_btn_remove'),
                'There should not be a Remove Button  if bar field is False');
