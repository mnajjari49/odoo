...
QUnit.module('basic_fields', {
    ...
    QUnit.test('widget url: external url prefixed with http in href attribute (readonly mode)', async function (assert) {
        assert.expect(2);

        this.data.partner.fields.url1 = { string: "Url 1", type: "char", default: "www.example.com" };
        this.data.partner.fields.url2 = { string: "Url 2", type: "char", default: "www.example.com" };
        ...
        assert.strictEqual(form.$('a[name="url1"]').attr('href'), 'http://www.example.com', 'Redirection link should prefixed with http');
        assert.strictEqual(form.$('a[name="url2"]').attr('href'), 'www.example.com', 'Redirection link should not prefixed with http and consider as internal link');
