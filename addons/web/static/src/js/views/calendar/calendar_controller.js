...
var CalendarController = AbstractController.extend({
     ...
     */
    _onButtonNavigation: function (jsEvent) {
        var action = this._extractLastPartOfClassName('o_calendar_button_', jsEvent.currentTarget.classList);
        if (action) {
            this._move(action);
        }
    },
     ...
     * @param {Event|jQueryEvent} jsEvent
     */
    _onButtonScale: function (jsEvent) {
        var scale = this._extractLastPartOfClassName('o_calendar_button_', jsEvent.currentTarget.classList);
        if (scale) {
