...
var BasicController = AbstractController.extend(FieldManagerMixin, {
    ...
    _getPagerProps: function () {
        ...
        return {
            ...
            minimum: (isGrouped ? state.groupsOffset : state.offset) + 1,
    ...
    _updateControlPanel: function () {
        const state = this.model.get(this.handle, { env: true });
    ...
    _onPagerChanged: async function (ev) {
        let { minimum, maximum, limit } = ev.detail;
        this.updateControlPanel({
            pager: { disabled: true },
        });
        const data = this.model.get(this.handle, { raw: true });
        const reloadParams = data.groupedBy && data.groupedBy.length ?
            { groupsLimit: limit, groupsOffset: minimum - 1 } :
            { limit: limit, offset: minimum - 1 };
        ...
        this.updateControlPanel({
            pager: { disabled: false, limit, maximum, minimum },
        });
    },
    /**
