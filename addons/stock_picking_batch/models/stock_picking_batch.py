...
class StockPickingBatch(models.Model):
    ...
    move_line_ids = fields.One2many(
        'stock.move.line', 'batch_id', string='Stock move lines', readonly=True,
        states={'draft': [('readonly', False)], 'in_progress': [('readonly', False)]}
        )
    ...
    def _compute_allowed_picking_ids(self):
        ...
        for batch in self:
            ...
            allowed_pickings = self.env['stock.picking'].search_read(domain, ['id'])
            allowed_picking_ids = [picking['id'] for picking in allowed_pickings]
            batch.allowed_picking_ids = [(6, False, allowed_picking_ids)]

    @api.depends('picking_ids', 'picking_ids.move_lines', 'picking_ids.move_lines.state')
