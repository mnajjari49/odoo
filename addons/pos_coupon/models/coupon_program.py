...
class CouponProgram(models.Model):
    ...
    def _compute_valid_product_ids(self):
        for program in self:
            if program.rule_products_domain == False:
                domain = []
            else:
    ...
    def _compute_valid_partner_ids(self):
        for program in self:
            if program.rule_partners_domain == False:
                domain = []
            else:
    ...
    def _discount_line_product_vals(self, name):
        # NOTE This looks like a bad design. We don't want duplicated code like this.
        # The problem is that we can't set taxes_id to be False in coupon module
        # because it is not dependent on account module. Perhaps it is better
        # for coupon module to depend on account in order to easily set taxes_id
        # to False.
        vals = super(CouponProgram, self)._discount_line_product_vals(name)
        vals.update({'taxes_id': False, 'supplier_taxes_id': False})
        return vals
