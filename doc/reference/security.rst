...
common mistakes leading to unsecure code.

Unsafe Public Methods
---------------------

Any public method can be execute via a :ref:`RPC call
<webservices/odoo/calling_methods>` with the chosen parameters. The methods
starting with a `_` are considered as private in Odoo and not callable from
an action button or external API.
...
.. code-block:: python
    ...
    # this method is private and can only be called from other python method
...
the final integration in the browser DOM. A `t-raw` that is correctly escaped at
the time of introduction may no longer be safe at the next bugfix or
refactoring.
...
pratice that may lead to unexpected security vulnerabilities once this code
...
While formating the template differently would prevent such vulnerabilities.
...
avoided at all cost. A safer, sandboxed, method :class:`~odoo.tools.safe_eval`
can be used instead but still gives tremendous capabilities to the user running
it and must be reserved for trusted priviledged users only as it breaks the
